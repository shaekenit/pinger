...
database_engine = create_engine(
    ServerConfig.DATABASE_URL, connect_args={"check_same_thread": False}, future=True
)
DatabaseSession = sessionmaker(
    bind=database_engine, autoflush=False, autocommit=False, future=True
)
...
class ConnectionManager:
    ...
    async def send_connected_clients(self):
        ...
        for username, websocket in list(self.active_connections.items()):
            try:
                await websocket.send_json({"type": "clientlist", "clients": usernames})
            except Exception:
...
class RateLimiter:
    def __init__(self, requests_per_minute: int):
        ...
        self.burst_multiplier = ServerConfig.RATE_LIMIT_BURST_MULTIPLIER
        self.capacity = self.requests_per_minute * self.burst_multiplier
        self.refill_per_second = self.requests_per_minute / 60.0
        self.buckets: Dict[str, Dict[str, float]] = {}
        self.per_target_last: Dict[Tuple[str, str], float] = {}
        self.per_target_cooldown = ServerConfig.PER_TARGET_COOLDOWN_SECONDS
    ...
    def allow(
        self, username: str, to_username: Optional[str] = None
    ) -> Tuple[bool, int]:
        now = time.time()

        bucket = self.buckets.get(username)
        if not bucket:
            bucket = {"tokens": self.capacity, "last": now}
            self.buckets[username] = bucket
        ...
        if bucket["tokens"] < 1.0:
            ...
            secs = (
                tokens_needed / self.refill_per_second
                if self.refill_per_second > 0
                else 1.0
            )
            retry_after = max(1, int(math.ceil(secs)))
            return False, retry_after

        bucket["tokens"] -= 1.0

        if to_username:
            key = (username, to_username)
            last = self.per_target_last.get(key, 0.0)
            since = now - last
            if since < self.per_target_cooldown:
                retry_after = int(math.ceil(self.per_target_cooldown - since))
                return False, retry_after
            self.per_target_last[key] = now
    ...
    def cleanup_old_counters(self) -> None:
        ...
        stale_users = [
            u for u, b in self.buckets.items() if current_time - b.get("last", 0) > 3600
        ]
        for u in stale_users:
            self.buckets.pop(u, None)

        stale_targets = [
            k for k, ts in self.per_target_last.items() if current_time - ts > 3600
        ]
        for k in stale_targets:
            self.per_target_last.pop(k, None)
...
class PingService:
    ...
    def _store_ping_record(
        self, from_username: str, to_username: str, timestamp: float, delivered: bool
    ) -> None:
        with DatabaseSession() as db_session:
            db_session.add(
                PendingPing(
...
app = FastAPI(title="Ping Middleman Server", lifespan=lifespan)
...
async def login_user(credentials: dict):
    ...
    if not username or not isinstance(username, str):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Valid username required"
        )

    token = AuthenticationService.generate_token()
    expiry_time = (
        AuthenticationService.get_current_time() + ServerConfig.TOKEN_EXPIRY_SECONDS
    )

    with DatabaseSession() as db_session:
        db_session.add(SessionToken(token=token, username=username, expiry=expiry_time))
        db_session.commit()
...
async def send_user_ping(
    ping_request: dict, sender_username: str = Depends(get_current_user)
):
    target_username = ping_request.get("to")
    if not target_username or not isinstance(target_username, str):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Target username required"
        )

    allowed, retry_after = rate_limiter.allow(sender_username, target_username)
    if not allowed:
        raise HTTPException(
        ...
        while True:
            try:
                ...
                if message.strip().lower() == "ping":
                    allowed, _ = rate_limiter.allow(username)
                    if allowed:
...
async def health_check():
    with DatabaseSession() as db_session:
        queued_count = (
            db_session.query(PendingPing).filter(PendingPing.delivered == False).count()
        )
